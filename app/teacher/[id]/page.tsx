...
export default function TeacherPage({
  ...
  if (!teacher) {
    return (
      <main className="mx-auto max-w-2xl px-4 pt-10">
        <PageHeader title="المعلّمة غير موجودة" back="/teachers" />
      </main>
    );
  }

  const herHalaqas = halaqas.filter((h) => teacher.halaqaIds.includes(h.id));
  const herStudents = students.filter((s) => s.teacherId === id);
  ...
  const saveEdit = () => {
    ...
    actions.updateTeacher(id, { name: editName.trim(), halaqaIds: editHalaqas });
    setEditing(false);
  };

  return (
    <main className="mx-auto max-w-2xl px-4 pb-28 pt-8">
      <PageHeader title={`المعلّمة ${teacher.name}`} back="/teachers" />
      ...
      >
        ...
        <PrimaryBtn
          onClick={() => {
            ...
            actions.addStudent(newName, adding, id);
            setAdding(null);
          }}
        >
          إضافة
